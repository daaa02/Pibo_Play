...
class Soc():
    ...
    def soc_4(self):
        ...
        while True:            
            ...
            pibo = cm.tts(bhv="do_agree", string=f"그럴 수 있겠구나. 파이보는 빨리 움직일 수 있는 {wm.word(self.user_name, 0)}가 너무 부러워~")
            break
            
            
        pibo = cm.tts(bhv="do_stop", string=f"{wm.word(self.user_name, 0)}가 열심히 놀이를 했으니, 오늘은 바른 스탬프를 찍어줄게.")
        behavior.do_stamp()
        time.sleep(1)
